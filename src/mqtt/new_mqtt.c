...
#define BUILD_AND_VERSION_FOR_MQTT                                             \
  "Open" PLATFORM_MCU_NAME " " USER_SW_VER " " __DATE__ " " __TIME__
...
static int mbedtls_verify_cb(void *data, mbedtls_x509_crt *crt, int depth,
                             uint32_t *flags);
...
// constant value, how much interval between self state broadcast (enabled by
// flag) You can change it with command: mqtt_broadcastInterval 60
...
static void MQTT_Mutex_Free() { xSemaphoreGive(g_mutex); }
...
// are working...
int MQTT_Post_Received(const char *topic, int topiclen,
                       const unsigned char *data, int datalen) {
  MQTT_Mutex_Take(100);
  if ((MQTT_RX_BUFFER_MAX - 1 - mqtt_rx_buffer_count) <
      topiclen + datalen + 2 + 2) {
    addLogAdv(LOG_ERROR, LOG_FEATURE_MQTT,
              "MQTT_rx buffer overflow for topic %s", topic);
  } else {
...
int MQTT_Post_Received_Str(const char *topic, const char *data) {
  return MQTT_Post_Received(topic, strlen(topic), (const unsigned char *)data,
                            strlen(data));
}
int get_received(char **topic, int *topiclen, unsigned char **data,
                 int *datalen) {
  int res = 0;
  MQTT_Mutex_Take(100);
...
int g_MqttPublishItemsQueued = 0; // Items in the queue waiting to be published.
                                  // This is not the queue length.
...
// static bool g_firstFullBroadcast = true;  //Flag indicating that we need to
// do a full broadcast
...
int MQTT_GetConnectEvents(void) { return mqtt_connect_events; }

int MQTT_GetPublishEventCounter(void) { return mqtt_published_events; }

int MQTT_GetPublishErrorCounter(void) { return mqtt_publish_errors; }

int MQTT_GetReceivedEventCounter(void) { return mqtt_received_events; }

int MQTT_GetConnectResult(void) { return mqtt_connect_result; }

// Based on mqtt_connection_status_t and
...
const char *get_callback_error(int reason) {
  ...
  case MQTT_CONNECT_REFUSED_USERNAME_PASS:
    return "Refused user credentials";
...
char *MQTT_GetStatusMessage(void) { return mqtt_status_message; }
...
int MQTT_RegisterCallback(const char *basetopic, const char *subscriptiontopic,
                          int ID, mqtt_callback_fn callback) {
  int index;
  int i;
  ...
  addLogAdv(LOG_INFO, LOG_FEATURE_MQTT,
            "MQTT_RegisterCallback called for bT %s subT %s", basetopic,
            subscriptiontopic);
  ...
  if (!callbacks[index]->subscriptionTopic ||
      strcmp(callbacks[index]->subscriptionTopic, subscriptiontopic)) {
    if (callbacks[index]->subscriptionTopic) {
      os_free(callbacks[index]->subscriptionTopic);
    }
    callbacks[index]->subscriptionTopic =
        (char *)os_malloc(strlen(subscriptiontopic) + 1);
 ...
 *  @return 		The topic without the client, or NULL if <client>/
 * wasn't present
 */
...
int channelGet(obk_mqtt_request_t *request) {
  ...
  // we only support here publishes with emtpy value, otherwise we would get
  // into a loop where we receive a get, and then send get reply with val, and
  // receive our own get
  if (request->receivedLen) {
    return 1;
  }

  addLogAdv(LOG_DEBUG, LOG_FEATURE_MQTT, "channelGet topic %i with arg %s",
  ...
  // atoi won't parse any non-decimal chars, so it should skip over the rest of
  // the topic.
  channel = atoi(p);
  ...
  for (i = 1; i < r; i++) {
    ...
    if (state_control == CONTROL) {
      if (strcmp(tokenStrValue, "data") == 0) {
        if (tryGetTokenString(json_str, &t[i + 1], tokenStrValue) == true) {
          ...
          if (strcmp(tokenStrValue, "open") == 0) {
            ...
            i += t[i + 1].size + 1;
            if (tryGetTokenString(json_str, &t[i + 1], tokenStrValue) &&
                strcmp(tokenStrValue, "value") == 0) {
      ...
      if (strcmp(tokenStrValue, "data") == 0) {
        ...
        addLogAdv(LOG_INFO, LOG_FEATURE_MQTT, "Enable value: %d",
                  enabled_value);
        ...
        addLogAdv(LOG_INFO, LOG_FEATURE_MQTT, "Recurrent value: %d",
                  recurrent_value);
      ...
      if (strcmp(tokenStrValue, "time") == 0) {
        int journey_time;
      ...
      i += t[i + 1].size + 1;
    } else if (state_control == SET_EMERGENCY_OPEN) {
      if (strcmp(tokenStrValue, "state") == 0) {
        int set_emergency;
        set_emergency = atoi(json_str + t[i + 1].start);
        CHANNEL_Set(EMERGENCY_OPEN, set_emergency, 0);
      }
      i += t[i + 1].size + 1;
      ...
      // addLogAdv(LOG_INFO, LOG_FEATURE_MQTT, "Unexpected key: %.*s", t[i].end
      // - t[i].start, 	json_str + t[i].start);
    }
    if (strcmp(tokenStrValue, "type") == 0) {
      if (tryGetTokenString(json_str, &t[i + 1], tokenStrValue) == true) {
        if (!strcmp(tokenStrValue, "get_states")) {
          ...
          MQTT_PublishTopicToClientData(mqtt_client, CFG_GetMQTTGroupTopic(),
                                        msg);
          os_free(msg);
          os_free(json_str);
          os_free(p);
...
void MQTT_PublishPrinterContentsToStat(obk_mqtt_publishReplyPrinter_t *printer,
                                       const char *statName) {
  const char *toUse;
...
void MQTT_PublishPrinterContentsToTele(obk_mqtt_publishReplyPrinter_t *printer,
                                       const char *statName) {
  const char *toUse;
  if (printer->allocated)
    toUse = printer->allocated;
...
void MQTT_ProcessCommandReplyJSON(const char *cmd, const char *args,
                                  int flags) {
  obk_mqtt_publishReplyPrinter_t replyBuilder;
  memset(&replyBuilder, 0, sizeof(obk_mqtt_publishReplyPrinter_t));
  JSON_ProcessCommandReply(cmd, args, &replyBuilder, (jsonCb_t)mqtt_printf255,
                           flags);
...
static OBK_Publish_Result MQTT_PublishTopicToClient(mqtt_client_t *client,
                                                    const char *sTopic,
                                                    const char *sChannel,
                                                    const char *sVal, int flags,
                                                    bool appendGet) {
  err_t err;
  ...
  if (flags & OBK_PUBLISH_FLAG_MUTEX_SILENT) {
    ...
    if (MQTT_Mutex_Take(500) == 0) {
      addLogAdv(LOG_ERROR, LOG_FEATURE_MQTT,
                "MQTT_PublishTopicToClient: mutex failed for %s=%s\r\n",
                sChannel, sVal);
      return OBK_PUBLISH_MUTEX_FAIL;
    }
  }
  ...
  // pub_topic = (char*)os_malloc(strlen(sTopic) + 1 + strlen(sChannel) + 5 +
  // 1); //5 for /get
  ...
  if ((pub_topic != NULL) && (sVal != NULL)) {
    ...
    // else
    // {
    // 	sprintf(pub_topic, "%s/%s%s", sTopic, sChannel, (appendGet == true ?
    // "/get" : ""));
    // }
    // if (sVal_len < 128)
    // {
    // 	addLogAdv(LOG_INFO, LOG_FEATURE_MQTT, "Publishing val %s to %s
    // retain=%i\n", sVal, pub_topic, retain);
    // }
    // else {
    // 	addLogAdv(LOG_INFO, LOG_FEATURE_MQTT, "Publishing val (%d bytes) to %s
    // retain=%i\n", sVal_len, pub_topic, retain);
    // }

    LOCK_TCPIP_CORE();
    if (appendGet == true) {
      pub_data = (char *)os_malloc(
          strlen("[{\"id\":\"switch.\",\"state\":\"\"}]") + strlen(sChannel) +
          (strcmp(sVal, "0") == 0 ? 3 : 2) + 1);
      sprintf(pub_data, "[{\"id\":\"switch.%s\",\"state\":\"%s\"}]", sChannel,
              (strcmp(sVal, "0") == 0 ? "off" : "on"));
      err = mqtt_publish(client, pub_topic, pub_data, strlen(pub_data), qos,
                         retain, mqtt_pub_request_cb, 0);
    } else {
      err = mqtt_publish(client, pub_topic, sVal, strlen(sVal), qos, retain,
...
// This is used to publish channel values in "obk0696FB33/1/get" format with
// numerical value, This is also used to publish custom information with string
// name, for example, "obk0696FB33/voltage/get" is used to publish voltage from
// the sensor

static OBK_Publish_Result MQTT_PublishTopicToClientData(mqtt_client_t *client,
                                                        const char *sTopic,
                                                        const char *sVal) {
  int flags = 0;
  bool appendGet = true;
  ...
  if ((pub_topic != NULL)) {
    ...
    if (err != ERR_OK) {
      if (err == ERR_CONN) {
        // addLogAdv(LOG_ERROR, LOG_FEATURE_MQTT, "Publish err: ERR_CONN aka
        // %d\n", err);
      } else if (err == ERR_MEM) {
        // addLogAdv(LOG_ERROR, LOG_FEATURE_MQTT, "Publish err: ERR_MEM aka
        // %d\n", err);
        g_memoryErrorsThisSession++;
      } else {
        // addLogAdv(LOG_ERROR, LOG_FEATURE_MQTT, "Publish err: %d\n", err);
...
static OBK_Publish_Result MQTT_PublishMain(mqtt_client_t *client,
                                           const char *sChannel,
                                           const char *sVal, int flags,
                                           bool appendGet) {
  return MQTT_PublishTopicToClient(mqtt_client, CFG_GetMQTTClientId(), sChannel,
                                   sVal, flags, appendGet);
}
OBK_Publish_Result MQTT_PublishTele(const char *teleName,
                                    const char *teleValue) {
  char topic[64];
  snprintf(topic, sizeof(topic), "tele/%s", CFG_GetMQTTClientId());
  return MQTT_PublishTopicToClient(mqtt_client, topic, teleName, teleValue, 0,
                                   false);
}
OBK_Publish_Result MQTT_ReturnState() {
  ...
  cJSON_AddNumberToObject(json, "door_sensor",
                          (CHANNEL_Get(DOOR_SENS) ? 0 : 1));
  ...
  cJSON_AddNumberToObject(json, "fire_alarm", CHANNEL_Get(FIRE_ALARM));
  cJSON_AddNumberToObject(json, "ui_mode", CHANNEL_Get(UI_MODE));
  cJSON_AddNumberToObject(json, "emergency_open", CHANNEL_Get(EMERGENCY_OPEN));
...
#ifdef JWGB2
  cJSON_AddNumberToObject(json, "disable_remote", CHANNEL_Get(DISABLE_REMOTE));
  cJSON_AddNumberToObject(json, "disable_remote_start_time",
                          CHANNEL_Get(DISABLE_REMOTE_START_TIME));
  cJSON_AddNumberToObject(json, "disable_remote_end_time",
                          CHANNEL_Get(DISABLE_REMOTE_END_TIME));
#endif
...
OBK_Publish_Result MQTT_ReturnSys() {
  ...
  // sprintf(dataStr,
  // "{\"model\":\"JWGP\",\"version\":\"1.0.5\",\"build\":63,\"hardware\":\"JWGP_BEKEN_WB3S_IO_01678ABENO\",\"ip\":\"%s\",\"mac\":\"%s\",\"netid\":\"%"PRIu64"\",\"wifi_name\":\"%s\",\"mqtt\":{\"broker\":\"%s\",\"port\":%d,\"username\":\"%s\",\"password\":\"%s\"},\"privatekey\":\"%s\"}",
  // HAL_GetMyIPString(), HAL_GetMACStrn(macstr), num, CFG_GetWiFiSSID(),
  // CFG_GetMQTTHostLocal(), CFG_GetMQTTPort(), CFG_GetMQTTUserNameLocal(),
  // CFG_GetMQTTPassLocal(),CFG_GetMQTTPass());
  cJSON *json = cJSON_CreateObject();
...
OBK_Publish_Result MQTT_PublishStat(const char *statName,
  ...
  return MQTT_PublishTopicToClient(mqtt_client, topic, statName, statValue, 0,
                                   false);
}
...
/// @param flags
/// @return
OBK_Publish_Result MQTT_Publish(const char *sTopic, const char *sChannel,
                                const char *sVal, int flags) {
  return MQTT_PublishTopicToClient(mqtt_client, sTopic, sChannel, sVal, flags,
                                   false);
}

void MQTT_OBK_Printf(char *s) { addLogAdv(LOG_INFO, LOG_FEATURE_MQTT, s); }
...
static void mqtt_incoming_data_cb(void *arg, const u8_t *data, u16_t len,
  ...
  // const struct mqtt_connect_client_info_t* client_info = (const struct
  // mqtt_connect_client_info_t*)arg;

  // if we stored a topic in g_mqtt_request, then we found a matching callback,
  // so use it.
  if (g_mqtt_request.topic[0]) {
    ...
    // addLogAdv(LOG_INFO, LOG_FEATURE_MQTT, "MQTT in topic %s",
    // g_mqtt_request.topic);
    ...
    for (i = 0; i < numCallbacks; i++) {
      ...
      if (!strncmp(g_mqtt_request.topic, cbtopic, strlen(cbtopic))) {
        MQTT_Post_Received(g_mqtt_request.topic, strlen(g_mqtt_request.topic),
                           data, len);
        // if ANYONE is interested, store it.
        break;
        // note - callback must return 1 to say it ate the mqtt, else further
        // processing can be performed. i.e. multiple people can get each topic
        // if required.
    ...
    // addLogAdv(LOG_INFO, LOG_FEATURE_MQTT, "MQTT topic not handled: %s",
    // g_mqtt_request.topic);
  }
}
    ...
    if (found) {
      ...
      for (int i = 0; i < numCallbacks; i++) {
        ...
        if (!strncmp(topic, cbtopic, strlen(cbtopic))) {
          // note - callback must return 1 to say it ate the mqtt, else further
          // processing can be performed. i.e. multiple people can get each
          // topic if required.
...
static void mqtt_connection_cb(mqtt_client_t *client, void *arg,
  ...
  //   addLogAdv(LOG_INFO,LOG_FEATURE_MQTT,"MQTT client < removed name >
  //   connection cb: status %d\n",  (int)status);
  //  addLogAdv(LOG_INFO,LOG_FEATURE_MQTT,"MQTT client \"%s\" connection cb:
  ...
  if (status == MQTT_CONNECT_ACCEPTED) {
    ...
    // this makes a BIG assumption that we can subscribe multiple times to the
    // same one?
    // TODO - check that subscribing multiple times to the same topic is not BAD
    for (i = 0; i < numCallbacks; i++) {
      if (callbacks[i]) {
        if (callbacks[i]->subscriptionTopic &&
            callbacks[i]->subscriptionTopic[0]) {
          err = mqtt_sub_unsub(client, callbacks[i]->subscriptionTopic, 1,
                               mqtt_request_cb,
                               LWIP_CONST_CAST(void *, client_info), 1);
          if (err != ERR_OK) {
            addLogAdv(LOG_INFO, LOG_FEATURE_MQTT,
                      "mqtt_subscribe to %s return: %d\n",
    ...
    // LOCK_TCPIP_CORE();
    err = mqtt_publish(client, tmp, "online", strlen("online"), 2, true,
                       mqtt_pub_request_cb, 0);
    // UNLOCK_TCPIP_CORE();
    if (err != ERR_OK) {
      addLogAdv(LOG_ERROR, LOG_FEATURE_MQTT, "Publish err: %d\n", err);
    ...
    addLogAdv(LOG_INFO, LOG_FEATURE_MQTT,
              "mqtt_connection_cb: Disconnected, reason: %d(%s)\n", status,
              get_callback_error(status));
  }
}
...
static int MQTT_do_connect(mqtt_client_t *client) {
  ...
  if (!mqtt_host[0]) {
    addLogAdv(LOG_INFO, LOG_FEATURE_MQTT,
              "mqtt_host empty, not starting mqtt\r\n");
    snprintf(mqtt_status_message, sizeof(mqtt_status_message),
             "mqtt_host empty, not starting mqtt");
    return 0;
  }
  ...
  if (dns_in_progress_time <= 0 && !dns_resolved) {
    addLogAdv(LOG_INFO, LOG_FEATURE_MQTT,
              ...
              // mqtt_pass,
              "********", mqtt_clientID, mqtt_host, mqtt_port);
  }
  ...
  if (NULL != hostEntry) {
    ...
    if (hostEntry->h_addr_list && hostEntry->h_addr_list[0]) {
      ...
      if (len > 4) {
        addLogAdv(LOG_INFO, LOG_FEATURE_MQTT,
                  "mqtt_host resolves to addr len > 4\r\n");
        len = 4;
      }
    ...
    {
      addLogAdv(LOG_INFO, LOG_FEATURE_MQTT,
                "mqtt_host resolves no addresses?\r\n");
      snprintf(mqtt_status_message, sizeof(mqtt_status_message),
               "mqtt_host resolves no addresses?");
      return 0;
    }
...
#else
    res = dns_gethostbyname_addrtype(mqtt_host, &mqtt_ip_resolved, dnsFound,
  ...
  if (dns_in_progress_time <= 0 && dns_resolved) {
    ...
    if (mqtt_use_tls) {
      ...
      if (mqtt_verify_tls_cert) {
        if (strlen(CFG_GetMQTTCertFile()) > 0) {
          addLogAdv(LOG_INFO, LOG_FEATURE_MQTT, "Load certificate %s",
                    CFG_GetMQTTCertFile());
      ...
      if (mqtt_client_info.tls_config) {
#if ALTCP_MBEDTLS_DEBUG
        mbedtls_ssl_conf_verify(&mqtt_client_info.tls_config->conf,
                                mbedtls_verify_cb, NULL);
#if MBEDTLS_DEBUG_C
        mbedtls_ssl_conf_dbg(&mqtt_client_info.tls_config->conf,
                             mbedtls_debug_cb, NULL);
        ...
        if (mqtt_verify_tls_cert) {
          mbedtls_ssl_conf_authmode(&mqtt_client_info.tls_config->conf,
        ...
        addLogAdv(LOG_INFO, LOG_FEATURE_MQTT,
                  "Secure TLS config fail. Try connect anyway.");
      }
    }
#endif /* MQTT_USE_TLS */
    ...
    if (res != ERR_OK) {
      addLogAdv(LOG_INFO, LOG_FEATURE_MQTT,
                ...
                get_error_name(res));
      snprintf(mqtt_status_message, sizeof(mqtt_status_message),
               "mqtt_client_connect connect failed");
      if (res == ERR_ISCONN) {
    ...
    if (dns_in_progress_time > 0) {
      addLogAdv(LOG_INFO, LOG_FEATURE_MQTT,
                "mqtt_host %s is being resolved by gethostbyname\r\n",
                mqtt_host);
      ...
      if (!dns_resolved) {
        addLogAdv(LOG_INFO, LOG_FEATURE_MQTT,
                  "mqtt_host %s not found by gethostbyname\r\n", mqtt_host);
        snprintf(mqtt_status_message, sizeof(mqtt_status_message),
                 "mqtt_host %s not found by gethostbyname", mqtt_host);
      }
    }
  }
  return 0;
...
OBK_Publish_Result MQTT_PublishMain_StringString(const char *sChannel,
                                                 const char *valueStr,
                                                 int flags) {
...
OBK_Publish_Result MQTT_ChannelPublish(int channel, int flags) {
  ...
  // allow users to force-hide some channels (those channels are NEVER
  // published)
  ...
  if (CFG_HasFlag(OBK_FLAG_PUBLISH_MULTIPLIED_VALUES)) {
    ...
    // Float value
    addLogAdv(LOG_INFO, LOG_FEATURE_MQTT,
              "Channel has changed! Publishing %f to channel %i \n", dVal,
              channel);
    ...
    // Integer value
    addLogAdv(LOG_INFO, LOG_FEATURE_MQTT,
              "Channel has changed! Publishing %i to channel %i \n", iVal,
              channel);
...
// This console command will trigger a publish of all used variables (channels
// and extra stuff)
commandResult_t MQTT_PublishAll(const void *context, const char *cmd,
                                const char *args, int cmdFlags) {
  MQTT_PublishWholeDeviceState_Internal(true);
  return CMD_RES_OK; // TODO make return values consistent for all console
                     // commands
}
// This console command will trigger a publish of runtime variables
commandResult_t MQTT_PublishChannels(const void *context, const char *cmd,
                                     const char *args, int cmdFlags) {
  MQTT_PublishOnlyDeviceChannelsIfPossible();
  return CMD_RES_OK; // TODO make return values consistent for all console
                     // commands
}
commandResult_t MQTT_PublishChannel(const void *context, const char *cmd,
                                    const char *args, int cmdFlags) {
  int channelIndex;
...
commandResult_t MQTT_PublishCommand(const void *context, const char *cmd,
                                    const char *args, int cmdFlags) {
  ...
  Tokenizer_TokenizeString(args, TOKENIZER_ALLOW_QUOTES |
                                     TOKENIZER_ALLOW_ESCAPING_QUOTATIONS |
                                     TOKENIZER_EXPAND_EARLY);

  if (Tokenizer_GetArgsCount() < 2) {
    addLogAdv(LOG_INFO, LOG_FEATURE_MQTT,
              "Publish command requires two arguments (topic and value)");
    return CMD_RES_NOT_ENOUGH_ARGUMENTS;
  }
...
*/
#if ENABLE_LITTLEFS
commandResult_t MQTT_PublishFile(const void *context, const char *cmd,
                                 const char *args, int cmdFlags) {
...
#endif
// we have a separate command for integer because it can support math
// expressions (so it handled like $CH10*10, etc)
commandResult_t MQTT_PublishCommandInteger(const void *context, const char *cmd,
                                           const char *args, int cmdFlags) {
  const char *topic;
  int value;
  OBK_Publish_Result ret;
...
commandResult_t MQTT_PublishCommandFloat(const void *context, const char *cmd,
                                         const char *args, int cmdFlags) {
  const char *topic;
  float value;
  OBK_Publish_Result ret;
...
commandResult_t MQTT_PublishCommandDriver(const void *context, const char *cmd,
                                          const char *args, int cmdFlags) {
  const char *driver;
  OBK_Publish_Result ret;
  ...
  if (info != NULL) {
    while (1) {
      ...
      if (info->msg_cnt < info->msg_num) {
        sprintf(info->value, "TestMSG: %li/%li Time: %i s, Rate: %i msg/s",
                info->msg_cnt, info->msg_num, (int)info->bench_time,
                (int)info->bench_rate);
        LOCK_TCPIP_CORE();
        err = mqtt_publish(mqtt_client, info->topic, info->value,
                           strlen(info->value), qos, retain,
                           mqtt_pub_request_cb, 0);
        UNLOCK_TCPIP_CORE();
        if (err == ERR_OK) {
          /* MSG published */
        ...
        if (info->report_published == false) {
          /* Publish report */
          sprintf(info->value,
                  "Benchmark completed. %li msg published. Total Time: %i s "
                  "MsgRate: %i msg/s",
...
commandResult_t MQTT_SetTasTeleIntervals(const void *context, const char *cmd,
                                         const char *args, int cmdFlags) {
  Tokenizer_TokenizeString(args, 0);
...
commandResult_t MQTT_SetMaxBroadcastItemsPublishedPerSecond(const void *context,
                                                            const char *cmd,
                                                            const char *args,
                                                            int cmdFlags) {
  Tokenizer_TokenizeString(args, 0);
...
commandResult_t MQTT_SetBroadcastInterval(const void *context, const char *cmd,
                                          const char *args, int cmdFlags) {
  Tokenizer_TokenizeString(args, 0);
  // following check must be done after 'Tokenizer_TokenizeString',
  // so we know arguments count in Tokenizer. 'cmd' argument is
...
static BENCHMARK_TEST_INFO *info = NULL;
    ...
    PLATFORM_TR6260 || PLATFORM_REALTEK || PLATFORM_ECR6600 ||                 \
    PLATFORM_ESP8266 || PLATFORM_TXW81X || PLATFORM_RDA5981 || PLATFORM_LN8825
...
commandResult_t MQTT_StartMQTTTestThread(const void *context, const char *cmd,
                                         const char *args, int cmdFlags) {
  if (info != NULL) {
...
#elif PLATFORM_BL602 || PLATFORM_W600 || PLATFORM_W800 || PLATFORM_ESPIDF ||   \
    PLATFORM_TR6260 || PLATFORM_REALTEK || PLATFORM_ECR6600 ||                 \
    PLATFORM_ESP8266 || PLATFORM_LN8825
  ...
  if (OS_TimerCreate(&timer, OS_TIMER_PERIODIC, MQTT_Test_Tick, (void *)info,
                     MQTT_TMR_DURATION) != OS_OK) {
  ...
  result = rtos_init_timer(&g_mqtt_timer, MQTT_TMR_DURATION, MQTT_Test_Tick,
                           (void *)info);
...
void MQTT_InitCallbacks() {
  ...
  // so-called "Group topic", a secondary topic that can be set on multiple
  // devices to control them together register the TAS cmnd callback if
  // (*groupId) {
  // 	// register the main set channel callback
  ...
  // so-called "Group topic", a secondary topic that can be set on multiple
  // devices to control them together register the TAS cmnd callback
  if (*groupId) {
...
void MQTT_init() {
  ...
  // cmddetail:"descr":"Publishes data by MQTT. The final topic will be
  // obk0696FB33/[Topic]/get, but you can also publish under raw topic, by
  // adding third argument - '1'. You can use argument expansion here, so $CH11
  // will change to value of the channel 11",
  ...
  // obk0696FB33/[Topic]/get, but you can also publish under raw topic, by
  // adding third argument - '1'.. You can use argument expansion here, so $CH11
  // will change to value of the channel 11. This version of command publishes
  // an integer, so you can also use math expressions like $CH10*10, etc.",
  ...
  // adding third argument - '1'.. You can use argument expansion here, so $CH11
  // will change to value of the channel 11. This version of command publishes
  // an float, so you can also use math expressions like $CH10*0.0, etc.",
  ...
  // cmddetail:"descr":"Starts the step by step publish of all available
  // values",
  ...
  // cmddetail:"descr":"If broadcast self state every 60 seconds/minute is
  // enabled in flags, this value allows you to change the delay, change this 60
  // seconds to any other value in seconds. This value is not saved, you must
  // use autoexec.bat or short startup command to execute it on every reboot.",
  // cmddetail:"fn":"MQTT_SetBroadcastInterval","file":"mqtt/new_mqtt.c","requires":"",
  // cmddetail:"examples":""}
  CMD_RegisterCommand("mqtt_broadcastInterval", MQTT_SetBroadcastInterval,
                      NULL);
  // cmddetail:{"name":"mqtt_broadcastItemsPerSec","args":"[PublishCountPerSecond]",
  // cmddetail:"descr":"If broadcast self state (this option in flags) is
  // started, then gradually device info is published, with a speed of N
  // publishes per second. Do not set too high value, it may overload LWIP MQTT
  // library. This value is not saved, you must use autoexec.bat or short
  // startup command to execute it on every reboot.",
  // cmddetail:"fn":"MQTT_SetMaxBroadcastItemsPublishedPerSecond","file":"mqtt/new_mqtt.c","requires":"",
  // cmddetail:"examples":""}
  CMD_RegisterCommand("mqtt_broadcastItemsPerSec",
                      MQTT_SetMaxBroadcastItemsPublishedPerSecond, NULL);
  // cmddetail:{"name":"TasTeleInterval","args":"[SensorInterval][StateInterval]",
  // cmddetail:"descr":"This allows you to configure Tasmota TELE publish
  // intervals, only if you have TELE flag enabled. First argument is interval
  // for sensor publish (energy metering, etc), second is interval for State
  // tele publish.",
  ...
  // cmddetail:"descr":"Publishes data read from LFS file by MQTT. The final
  // topic will be obk0696FB33/[Topic]/get, but you can also publish under raw
  // topic, by adding third argument - '1'.",
...
static float getInternalTemperature() { return g_wifi_temperature; }

OBK_Publish_Result MQTT_DoItemPublishString(const char *sChannel,
                                            const char *valueStr) {
  return MQTT_PublishMain(mqtt_client, sChannel, valueStr,
                          OBK_PUBLISH_FLAG_MUTEX_SILENT, false);
}
...
*/
    ...
    // while all other platforms will accept uint32_t as long unsigned, ESP8266
    // needs %u biuild fails otherwise because of -Werror=format
    // src/mqtt/new_mqtt.c:2036:24: error: format '%ld' expects argument of type
    // 'long int', but argument 3 has type 'uint32_t' {aka 'unsigned int'}
    // [-Werror=format=] al other ESP:
    /// src/mqtt/new_mqtt.c:2036:44: error: format '%d' expects argument of type
  ...
  case PUBLISHITEM_SELF_IP:
    // g_firstFullBroadcast = false; //We published the last status item,
    // disable full broadcast
  ...
  // Do not publish raw channel value for channels like PWM values, RGBCW has 5
  // raw channels. We do not need raw values for RGBCW lights (or RGB, etc)
  ...
  if (bWantsToPublish) {
    return MQTT_ChannelPublish(g_publishItemIndex,
                               OBK_PUBLISH_FLAG_MUTEX_SILENT);
  }
...
int MQTT_RunEverySecondUpdate() {
  ...
  if (g_mqtt_bBaseTopicDirty) {
    addLogAdv(
        ...
        "MQTT base topic is dirty, will reinit callbacks and reconnect\n");
    MQTT_InitCallbacks();
    mqtt_reconnect = 5;
  }
  ...
  if (mqtt_reconnect > 0) {
    ...
    addLogAdv(LOG_INFO, LOG_FEATURE_MQTT, "MQTT has pending reconnect in %i\n",
              mqtt_reconnect);
    if (mqtt_reconnect == 0) {
      // then if connected, disconnect, and then it will reconnect automatically
      // in 2s
      if (mqtt_client && res) {
        addLogAdv(LOG_INFO, LOG_FEATURE_MQTT,
                  "MQTT will now do a forced reconnect\n");
        MQTT_disconnect(mqtt_client);
        mqtt_loopsWithDisconnected = LOOPS_WITH_DISCONNECTED - 2;
      }
    }
  ...
  if (mqtt_client == 0 || res == 0) {
    // addLogAdv(LOG_INFO,LOG_FEATURE_MAIN, "Timer discovers disconnected mqtt
    ...
    // Handle only queued items. Don't need to do this separately if entire
...
void MQTT_QueuePublishWithCommand(const char *topic, const char *channel,
                                  const char *value, int flags,
                                  PostPublishCommands command) {
  MqttPublishItem_t *newItem;
  if (g_MqttPublishItemsQueued >= MQTT_MAX_QUEUE_SIZE) {
    addLogAdv(LOG_ERROR, LOG_FEATURE_MQTT,
              "Unable to queue! %i items already present\r\n",
  ...
  if ((strlen(topic) > MQTT_PUBLISH_ITEM_TOPIC_LENGTH) ||
    ...
    addLogAdv(LOG_ERROR, LOG_FEATURE_MQTT,
              "Unable to queue! Topic (%i), channel (%i) or value (%i) exceeds "
              "size limit\r\n",
              strlen(topic), strlen(channel), strlen(value));
    return;
  }

  // Queue data for publish. This might be a new item in the queue or an
  // existing item. This is done to prevent memory fragmentation. The total
  // queue length is limited to MQTT_MAX_QUEUE_SIZE.
  ...
  g_MqttPublishItemsQueued++;
  addLogAdv(LOG_INFO, LOG_FEATURE_MQTT, "Queued topic=%s/%s, %i items in queue",
            newItem->topic, newItem->channel, g_MqttPublishItemsQueued);
}
...
void MQTT_InvokeCommandAtEnd(PostPublishCommands command) {
  ...
  if (tail == NULL) {
    addLogAdv(LOG_ERROR, LOG_FEATURE_MQTT,
              "InvokeCommandAtEnd invoked but queue is empty");
  } else {
...
/// @param value
/// @param flags
void MQTT_QueuePublish(const char *topic, const char *channel,
                       const char *value, int flags) {
...
OBK_Publish_Result PublishQueuedItems() {
  ...
  // The next actionable item might not be at the front. The queue size is
  // limited to MQTT_QUEUED_ITEMS_PUBLISHED_AT_ONCE so this traversal is fast.
  // addLogAdv(LOG_INFO,LOG_FEATURE_MQTT,"PublishQueuedItems
  // g_MqttPublishItemsQueued=%i",g_MqttPublishItemsQueued );
  while ((head != NULL) && (count < MQTT_QUEUED_ITEMS_PUBLISHED_AT_ONCE) &&
         (g_MqttPublishItemsQueued > 0)) {
    if (!MQTT_QUEUE_ITEM_IS_REUSABLE(head)) { // Skip reusable entries
      count++;
      result =
          MQTT_PublishTopicToClient(mqtt_client, head->topic, head->channel,
                                    head->value, head->flags, false);
      ...
      // addLogAdv(LOG_INFO,LOG_FEATURE_MQTT,"PublishQueuedItems item skipped
      // reusable");
    }
...
struct tm *mbedtls_platform_gmtime_r(const mbedtls_time_t *tt,
  ...
  if (!NTP_IsTimeSynced()) {
    ...
    if (log_gmtime_alt) {
      addLogAdv(LOG_INFO, LOG_FEATURE_NTP,
                "MBEDTLS: NTP not synchronized. Using compile time: "
...
static void mbedtls_debug_cb(void *ctx, int level, const char *file, int line,
  ...
  addLogAdv(LOG_ERROR, LOG_FEATURE_MQTT, "%s:%04d: |%d| %s", basename, line,
            level, str);
}

void mbedtls_dump_conf(mbedtls_ssl_config *conf, mbedtls_ssl_context *ssl) {
  if (ssl && ssl->handshake) {
    addLogAdv(LOG_DEBUG, LOG_FEATURE_MQTT, "HANDSHAKE CIPHER SUITE: %s",
              ssl->handshake->ciphersuite_info->name);
    switch (ssl->handshake->ciphersuite_info->key_exchange) {
    case MBEDTLS_KEY_EXCHANGE_NONE:
      addLogAdv(LOG_DEBUG, LOG_FEATURE_MQTT,
                "HANDSHAKE KEY EXCHANGE: MBEDTLS_KEY_EXCHANGE_NONE");
      break;
    case MBEDTLS_KEY_EXCHANGE_RSA:
      addLogAdv(LOG_DEBUG, LOG_FEATURE_MQTT,
                "HANDSHAKE KEY EXCHANGE: MBEDTLS_KEY_EXCHANGE_RSA");
      break;
    case MBEDTLS_KEY_EXCHANGE_DHE_RSA:
      addLogAdv(LOG_DEBUG, LOG_FEATURE_MQTT,
                "HANDSHAKE KEY EXCHANGE: MBEDTLS_KEY_EXCHANGE_DHE_RSA");
      break;
    case MBEDTLS_KEY_EXCHANGE_ECDHE_RSA:
      addLogAdv(LOG_DEBUG, LOG_FEATURE_MQTT,
                "HANDSHAKE KEY EXCHANGE: MBEDTLS_KEY_EXCHANGE_ECDHE_RSA");
      break;
    case MBEDTLS_KEY_EXCHANGE_ECDHE_ECDSA:
      addLogAdv(LOG_DEBUG, LOG_FEATURE_MQTT,
                "HANDSHAKE KEY EXCHANGE: MBEDTLS_KEY_EXCHANGE_ECDHE_ECDSA");
      break;
    case MBEDTLS_KEY_EXCHANGE_PSK:
      addLogAdv(LOG_DEBUG, LOG_FEATURE_MQTT,
                "HANDSHAKE KEY EXCHANGE: MBEDTLS_KEY_EXCHANGE_PSK");
      break;
    case MBEDTLS_KEY_EXCHANGE_DHE_PSK:
      addLogAdv(LOG_DEBUG, LOG_FEATURE_MQTT,
                "HANDSHAKE KEY EXCHANGE: MBEDTLS_KEY_EXCHANGE_DHE_PSK");
      break;
    case MBEDTLS_KEY_EXCHANGE_RSA_PSK:
      addLogAdv(LOG_DEBUG, LOG_FEATURE_MQTT,
                "HANDSHAKE KEY EXCHANGE: MBEDTLS_KEY_EXCHANGE_RSA_PSK");
      break;
    case MBEDTLS_KEY_EXCHANGE_ECDHE_PSK:
      addLogAdv(LOG_DEBUG, LOG_FEATURE_MQTT,
                "HANDSHAKE KEY EXCHANGE: MBEDTLS_KEY_EXCHANGE_ECDHE_PSK");
      break;
    case MBEDTLS_KEY_EXCHANGE_ECDH_RSA:
      addLogAdv(LOG_DEBUG, LOG_FEATURE_MQTT,
                "HANDSHAKE KEY EXCHANGE: MBEDTLS_KEY_EXCHANGE_ECDH_RSA");
      break;
    case MBEDTLS_KEY_EXCHANGE_ECDH_ECDSA:
      addLogAdv(LOG_DEBUG, LOG_FEATURE_MQTT,
                "HANDSHAKE KEY EXCHANGE: MBEDTLS_KEY_EXCHANGE_ECDH_ECDSA");
      break;
    case MBEDTLS_KEY_EXCHANGE_ECJPAKE:
      addLogAdv(LOG_DEBUG, LOG_FEATURE_MQTT,
                "HANDSHAKE KEY EXCHANGE: MBEDTLS_KEY_EXCHANGE_ECJPAKE");
      break;
    }
  }
